class Asurada < Formula
  desc "AI memory + cognition daemon that grows with the user"
  ...
  def caveats
    <<~EOS
      Asurada binaries are not yet code-signed with an Apple
      Developer ID. On first launch macOS Gatekeeper may show
      an "unidentified developer" warning. To allow it:

        xattr -d com.apple.quarantine "$(which asurada)"

      Or right-click the binary in Finder → Open → Open anyway.

      Quick start:
        asurada init
        export ASURADA_DATABASE_URL='postgresql://...'
        export ASURADA_USER_ID='you'
        asurada serve
    EOS
  end

  test do
    assert_match "asurada", shell_output("#{bin}/asurada --version")
  end
end
